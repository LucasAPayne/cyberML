...
def get_rel2id(datapath):
    ...
    for i in range(len(content)):
        a = content[i].split('\t')
        ids.append(int(a[0]))
        entity_names.append(a[-1][:-1])
    rel2id = dict(zip(entity_names, ids))
...
def get_ent2id(datapath):
    ...
    for i in range(len(content)):
        ...
        ids.append(int(a[0]))
        entity_names.append(a[-1][:-1])
    ent2id = dict(zip(entity_names, ids))
...
def load_data(datapath):
    ...
    Output
    training triples, number of entities, number of relations
    '''
    train_data = np.array(np.loadtxt('{}/{}'.format(datapath, 'train.del')), dtype=int)

    num_entities = np.max([np.max(train_data[:,0]), np.max(train_data[:,2])])+1
    num_relations = np.max(train_data[:,1])+1

    return train_data, num_entities, num_relations
